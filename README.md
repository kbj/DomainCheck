...
```
字典流 ──► DNS 预检 worker 池（-dns-concurrency 个并行 NS 查询）
              │  NS 命中 → 直接判定已注册（不打 WHOIS）
              ▼  无 NS →
          有界等待队列（-whois-queue）
              ▼
          WHOIS 串行消费者（节奏/退避/降级与 v2 完全一致）
```

对每个域名按以下顺序判断：

1. **DNS NS 预检**（worker 池并行执行，默认 5 个 goroutine，`-dns-concurrency`）:
   查询域名的 NS 记录——**有 NS 记录则必然已注册**，直接判定并跳过 WHOIS；
   每个 worker 独立遵守 `-dns-interval` 节奏（N 个 worker ≈ N 倍 DNS 吞吐）；
2. 无 NS 记录的域名进入**有界队列**，由**严格串行**的 WHOIS 消费者逐个查询
   （带重试与指数退避）；队列满时 DNS 预检 worker 自然阻塞（背压），
   千万级字典内存占用依然有界；
...
6. DNS 查询自身失败也有指数退避重试；WHOIS 之间的速率限制仍由 `-delay`
   控制（含 ±25% 抖动，查前补足等待实现），并行化只加速 DNS 侧，**WHOIS
   对 nic 服务器的请求节奏与串行版本完全一致**。
...
- **优雅处理 Ctrl+C / SIGTERM**：中断时保存进度后正常退出（退出码 130），
  再次运行时选择续扫即可。
- **分级查询间隔**：`-dns-interval`（默认 1s，最小 0.1s）是**每个 DNS worker**
  两次预检之间的节奏（N 个 worker 并行即 N 倍 DNS 吞吐，同时作为 DNS 重试
  退避基数）；`-delay` 设定值（含 ±25% 抖动）约束的是**两次 WHOIS 查询之间**
  的最小间隔——以「查前补足等待」实现，只作用于串行 WHOIS 队列。
  DNS 侧并行、WHOIS 侧串行，两条速率约束互不干扰。
...
| `-dict` | `dict/` 目录下的字典文件名 |
| `-delay` | 两次 **WHOIS 查询**之间的最小间隔（秒），默认 0（不限速）；以「查前补足等待」实现，实际间隔在设定值附近随机抖动 ±25%。只作用于串行 WHOIS 队列，与 DNS 侧并行互不干扰 |
...
**内存与磁盘开销**：扫描全程流式读取字典（O(1) 内存，不随字典规模增长），
状态记忆的内存占用 O(失败数)。千万级字典（26 字母 5 位组合，约 1188 万条）
实测进程峰值 RSS 约 13 MB（此前一次性载入全量字典并去重需约 1 GB 堆内存）；
元数据始终约 420 字节。对比旧版把全部结果存在单个 JSON 里反复全量重写的方案，
大字典下写入量降低数个数量级。

注意：流式读取不再对字典去重（原版 Python 也不去重）——字典中的重复行
会被当作不同域名重复查询；`-gen` 生成的字典本身不会有重复。
